...
import com.dynatrace.diagnostics.plugins.jmx.monitor.MessageBridge;
import com.dynatrace.diagnostics.plugins.jmx.monitor.SAF;
...
public class WeblogicMonitoring implements Monitor, WeblogicConstants {
	...
	Boolean RegexAppData;
	Boolean RegexAppState;
...
/*		properties.setSystemProfile(env.getConfigString(CONFIG_SYSTEM_PROFILE).trim());
		...
		properties.setRegexAppData(env.getConfigBoolean(CONFIG_REGEX_APPDATA));
		properties.setRegexAppState(env.getConfigBoolean(CONFIG_REGEX_APPSTATE));
		
		
		properties.setsslTrust(env.getConfigBoolean(CONFIG_WEBLOGIC_SSL));
		properties.setcustomTrust(env.getConfigString(CONFIG_WEBLOGIC_SSLPATH).trim());
		...
		properties.setSAF(env.getConfigBoolean(CONFIG_WEBLOGIC_SAFAGENT));
		...
		properties.setMBD(env.getConfigBoolean(CONFIG_WEBLOGIC_MDB));
		properties.setMessageBridgeServer(env.getConfigString(CONFIG_WEBLOGIC_MESSAGEBRIDGESERVER).trim());
		...
		if (threadPoolCount < 1) {
            threadPoolCount = 1;
            log.warning("Monitoring Thread count cannot be less then 1. Setting to: " + threadPoolCount);
        } 
		if (threadPoolCount > 10) {
            threadPoolCount = 10;
            log.warning("Monitoring Thread count cannot be greater then 10. Setting to: " + threadPoolCount);
        }
		...
		String mbData=properties.getMessageBridgeServer();
		...
		mbcs=properties.getMBD();
		saf=properties.getSAF();
		certPath=properties.getcustomTrust();
		sslMonitor=properties.getsslTrust();
		
		RegexAppData=properties.getRegexAppData();
		RegexAppState=properties.getRegexAppState();
		...
		if (isEmptyOrBlank(mbData)){
			mbData="nothingWasEntered";
		}
		
		mbServers=mbData.split("\\s+");
		...
		connection = wc.initConnection(host, port, username, password, sslMonitor, certPath);
            ...
            try {
                ...
                while (iter.hasNext()) {
                	...
                	if(jms){
                		...
                		JMS sumTask = new JMS(destR, connection, env, environment, jmsMod, jmsQueue);
                    ...
                    if(applicationData){
                    	...
                    	ApplicationData sumTask = new ApplicationData(destR, connection, env, environment, dataMonitor, RegexAppData);
                	...
                	if(applicationState){
                    	...
                    	ApplicationState sumTask = new ApplicationState(destR, connection, env, environment, deployedSingle, stateIgnore, RegexAppState);
                	...
                	Future<ArrayList<SAFStats>> safd = null;
                	if(saf){
                		log.info("Entering SAF Monitoring " + server_name);
                		SAF sumTask = new SAF(destR, connection, env, environment, agents);
                		safd = serviceThread.submit(sumTask);
                    	log.info("JMS - Job was submitted");
                	}
                	...
                	Future<ArrayList<MBStats>> mbd = null;
                	if(mbcs){
                		log.info("Entering Message Bridge Monitoring " + server_name);
                		MessageBridge sumTask = new MessageBridge(destR, connection, env, environment, mbServers);
                		mbd = serviceThread.submit(sumTask);
                    	log.info("JVM - Job was submitted");
                	}
...
// populate gathered dynamic measures in PWH
            		...
            		if(mbcs){
            			try{
            				ArrayList<MBStats> results = mbd.get(timeOut, TimeUnit.MILLISECONDS);
            				log.info("WLServer - Result size: " + results.size());
            				if (results.size() > 0) {
            					com.dynatrace.diagnostics.plugins.jmx.record.RecordMBMeasure DTAS = new com.dynatrace.diagnostics.plugins.jmx.record.RecordMBMeasure();
            					DTAS.WriteToDT(env, results);
            				}
            			}catch(TimeoutException e){
            				log.warning("WLServer - No response after " + timeOut + " milliseconds. Canceling the task.");
            		...
            		if(saf){
            			try{
            				ArrayList<SAFStats> results = safd.get(timeOut, TimeUnit.MILLISECONDS);
            				log.info("SAF - Result size: " + results.size());
            				if (results.size() > 0) {
            					com.dynatrace.diagnostics.plugins.jmx.record.RecordSAFMeasure DTAS = new com.dynatrace.diagnostics.plugins.jmx.record.RecordSAFMeasure();
            					DTAS.WriteToDT(env, results);
            				}
            			}catch(TimeoutException e){
            				log.warning("SAF - No response after " + timeOut + " milliseconds. Canceling the task.");
                	        safd.cancel(true);
                		}
            		}
            		if(jdbc){
