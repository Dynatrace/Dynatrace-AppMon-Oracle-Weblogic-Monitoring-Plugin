...
public class JMS implements Callable<ArrayList<JMSStats>>, WeblogicConstants {
    ...
    public JMS(ObjectName destR, MBeanServerConnection connection, MonitorEnvironment env, String environment, String [] safAgent, String[] jmsMod, String[] jmsQueue) {
    	envs = env;
        destRT = destR;
                ...
                for (int x = 0; x < appLength; x++) {
                    ...
                    try {
                        ...
                        while (q < 1) {	
                        	...
                        	k = 0;
                        	while (j < safSize){
                        		if (JMSName == safAgents[j].toString()){
                        			k=1;
                        			break;
                        		} 
                        		j++;
                        	...
                        	if (k == 1){
                            intIndex = jms.indexOf(safAgents[j].toString());
                            if (intIndex != -1) {
                            	if (log.isLoggable(Level.INFO)) {
                            		log.info("No messages to monitor for: " + safAgents[j].toString() + " on server " + name);
                            		log.info("No Information will be recorded to DT for the SAF queue.");
                            	}
                                 //SAF is shutdown when not in use.
                                 break;
                            }
